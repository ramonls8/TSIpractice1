...
public class AgenteAStar extends AbstractPlayer{
	...
	// another, and selects those with the lowest cumulative cost plus heuristics. It guarantees
	...
	public ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
		if (path.empty() && !stateObs.isGameOver()){
			...
			System.out.println("* Number of open nodes upon completion: " + openNodes.size());
			System.out.println("* Number of closed nodes upon completion: " + closedNodes.size());
		}
	...
	private ArrayList<Node> nodeChildren(Node father){
		...
		for (int i = 0; i < children.size(); i++){
			...
			// Heuristic with Manhattam distance
	...
	private boolean calculatePath(){
		...
		openNodes.add(new Node((int) avatarInitialPosition.x, (int) avatarInitialPosition.y, 0, (int) (Math.abs(avatarInitialPosition.x - portal.x) + Math.abs(avatarInitialPosition.y - portal.y)), null, false, false, initialBlueCapes, initialRedCapes, ACTIONS.ACTION_NIL));
		...
		while (!openNodes.isEmpty() && !(currentNode.x == portal.x && currentNode.y == portal.y)){
			...
			for (int i=0; i<children.size(); i++){
				// There is no need to update closed nodes since
				// the heuristic is admissible and monotonic.
				...
				if (!closedNodes.contains(children.get(i))){
					...
					// and the heuristic is admissible and monotonic).
					/*
					else {
		...
		if (!(currentNode.x == portal.x && currentNode.y == portal.y))
			...
			// Add the goal node to closed nodes
			closedNodes.add(currentNode);
